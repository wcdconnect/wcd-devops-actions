...
function Format-VersionConstraintLabel {
    param([string] $Declared, [string] $Resolved)
    if ([string]::IsNullOrWhiteSpace($Declared)) {
        return $null
    }
    # Strip range operators for display of simple floors: "1.0.169", "[1.0.169, )", etc.
    $clean = $Declared.Trim()
    if ($clean -match '^\d') {
        if ($Resolved -and $clean -ne $Resolved) {
            return ">=$clean"
        }
        return $null
    }
    if ($clean -match '^[\[\(]\s*([^,\s\]\)]+)') {
        $floor = $Matches[1]
        if ($Resolved -and $floor -ne $Resolved) {
            return ">=$floor"
        }
        return $null
    }
...
"@
    ...
    [void]$sb.AppendLine("// Source: $AssetsPath")
    ...
    foreach ($id in ($resolved.Keys | Sort-Object)) {
        $label = Format-NodeId -Id $id -Version $resolved[$id]
        $nodeLabel[$id] = $label
        $isDirect = $directIds.Contains($id)
        $fill = if ($isDirect) { '#c6f6d5' } else { '#e2e8f0' }
        [void]$sb.AppendLine("  `"$(Escape-DotLabel $label)`" [fillcolor=`"$fill`"];")
    }
    ...
    foreach ($e in ($edges | Sort-Object From, To)) {
        ...
        $toVer = $resolved[$e.To]
        $edgeLabel = Format-VersionConstraintLabel -Declared $e.Declared -Resolved $toVer
        if ($edgeLabel) {
            [void]$sb.AppendLine("  `"$(Escape-DotLabel $nodeLabel[$e.From])`" -> `"$(Escape-DotLabel $nodeLabel[$e.To])`" [style=dashed color=`"#4a5568`" label=`"$(Escape-DotLabel $edgeLabel)`"];")
        }
        else {
